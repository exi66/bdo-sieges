...
const { getNodesOwners, getCalpheonOwner, getBalenosOwner, getMediahOwner, getValenciaOwner } =
...
<template>
  <section class="flex flex-col gap-2">
    <fieldset class="border border-black/20 dark:border-white/20 rounded p-2">
      <legend class="mx-2 px-1 text-2xl text-black/70 dark:text-white/70">
        {{ $t('home.castles') }}
      </legend>
    ...
    <fieldset class="border border-black/20 dark:border-white/20 rounded p-2">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-1">
        <div
          class="rounded p-2 flex gap-2 bg-black/10 dark:bg-white/10"
          v-for="item in getNodesOwners"
          :key="item"
        >
